// Supabase 云同步客户端与鉴权封装
// 设计：本地优先（localStorage 即时读写），云端在登录后后台同步。
// 未配置 env / 未登录时，整个模块为「空操作」，应用回退纯本地模式。
